...
export class Aperture implements PropertyValue {
     ...
     * @param {number} value_
     */
    constructor(private readonly value_: number) {
        const name = Object.keys(Aperture.ID).find(
            (key) => Aperture.ID[key] === value_,
        );
        const formatAperture = (aperture: number) =>
            'f' + aperture.toFixed(1).replace(/\.0$/, '');
        if (name) {
            this.label_ = name;
            this.aperture_ = 0;
    ...
    static findNearest(
        valueOrLabel: number | string,
        filter?: (aperture: Aperture) => boolean,
    ): Aperture | null {
        let aperture: number;
