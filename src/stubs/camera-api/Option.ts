...
export class Option implements PropertyValue {
    ...
    constructor(
        ...
        const propertyLabel = Object.keys(CameraProperty.ID).find(
            (key) => CameraProperty.ID[key] === propertyID_,
        );
        if (propertyLabel && propertyLabel in Option) {
            // eslint-disable-next-line  @typescript-eslint/no-explicit-any
            const optionLabels = (Option as any)[propertyLabel];
            const optionLabel = Object.keys(optionLabels).find(
                (key) => optionLabels[key] === value_,
            );
            if (optionLabel) {
    ...
    static forLabel(label: string): Option | null {
        ...
        if (
            ...
            return new Option(
                propertyID,
                (Option as any)[propertyLabel][optionLabel],
            );
        }
        return null;
    }
